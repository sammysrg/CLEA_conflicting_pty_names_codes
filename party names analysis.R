...
Slovakia = file %>% filter(ctr_n=="Slovakia")

# Step 3: Identify cases with the same party code, but different party names
same_pty_diff_pty_n = Slovakia %>% group_by(pty, pty_n) %>% summarize(n=n()) %>% ungroup() %>% group_by(pty) %>% filter(n()>1)

# Step 4: Identify cases with the same party code, but different party names occuring in the same month and year
same_pty_diff_pty_n_same_mn_yr = Slovakia %>% mutate(pty_n = stringr::str_to_lower(pty_n)) %>% group_by(pty, pty_n, mn, yr) %>% filter(n()>1)

# Step 5: Identify cases with same party name, but different party codes
diff_pty_same_pty_n = Slovakia %>%  mutate(pty_n = stringr::str_to_lower(pty_n)) %>% group_by(pty, pty_n) %>% summarize(n=n()) %>% ungroup() %>% group_by(pty_n) %>% filter(n()>1) %>% arrange(pty_n)
